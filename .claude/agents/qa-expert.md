...
- **Group item counts**: Money = 4 (Expenses, SavingsGoals, Insights, MonthlyReport), Kitchen = 4 (Cooking, Recipes, Shopping, Inventory), Household = 3 since v1.2.2-dev (MaidTasks, Remind, Vendors — was 2 on v1.2.1), Personal = 2 (CycleTracker, BodyStats), System = 2 (Backup, Settings). Total = 15 items across groups (with Today standalone = 16 drawer entry points in UI, 13 registered `Drawer.Screen`s since Expenses/Cooking/Remind are bottom-tab routes nested under "Home"). If Personal grows to 9 in v1.3, the Personal count updates here — the other group counts stay fixed unless explicitly requested.
...
- **TodayScreen tile grid**: `src/screens/TodayScreen.tsx` must render EXACTLY 5 tiles. Grep the file for the `tiles` `useMemo` and confirm the returned array length is 5 and its `key`s are `money`, `kitchen`, `household`, `personal`, `system` in that order. No tile may navigate to a route that doesn't exist — only `Expenses`, `Cooking`, `Remind` (tabs) and `BodyStats`, `Settings` (drawer) are valid targets.
