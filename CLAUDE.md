...
## Current Version
**v1.2.5** (tagged 2026-04-19) — "Launch-Crash Hotfix". Emergency patch on v1.2.4. The v1.2.4 APK crashed on launch on some devices, traced to native-module init of `expo-blur` and/or `lottie-react-native`. Both are now **disabled at call-site**: BlurView removed from tab bar + 3 modal backdrops (replaced with solid theme fill + darker scrim `rgba(0,0,0,0.45)`); `LottieBox` renders the fallback emoji in place of every animation slot. Both packages remain in `package.json` for a future cautious re-enable after verifying on-device native init.
- v1.2.4 release theme: "Design Polish + Battery Audit" — four design libraries (BlurView, Lottie, Moti, Phosphor), swipeable list rows in 6 screens, battery-first Skeleton loaders on 7 screens, floating-pill Toast with icon + Moti spring, time-aware hero gradient on TodayScreen, micro-copy pass across 10 empty states, form keyboard flow, PrayerTimesScreen countdown-leak fix (`useFocusEffect`). **v1.2.4 APK `d718bd8a` DOES NOT LAUNCH on some devices — do not distribute.**
...
- `package.json`: name `forshe`, version `1.2.5`.
- Latest APK build: `ed6cce5f-5f15-49f3-8971-7aaa603c0bb6` (v1.2.5 hotfix, queued 2026-04-19, https://expo.dev/accounts/smartbzss/projects/forshe/builds/ed6cce5f-5f15-49f3-8971-7aaa603c0bb6).
- Previous APK `d718bd8a` (v1.2.4) **crashes on launch** — do not distribute.
- Previous released tag: `v1.2.3` on master (last confirmed-working APK: `2218683b` from v1.2.3).
