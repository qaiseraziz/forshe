...
## Current Version
**v1.2.7** (tagged 2026-04-20) — "Launch-Crash Root-Cause Fix". Actual fix for the v1.2.4 → v1.2.6 launch crashes: `moti@0.30.0` was built against `react-native-reanimated@3.11.0` and is incompatible with our installed `reanimated@4.2.1`. Importing `MotiView` crashed the JS bridge at module init. Fix: `MotiEnter` and `Toast` now render plain `View` — no animation. All previous hotfix disables stay (BlurView removed in v1.2.5, Lottie emoji-only in v1.2.5, Phosphor emoji-only in v1.2.6 + react-native-svg added as direct dep — those were red herrings but the disables don't hurt). All four packages (`expo-blur`, `lottie-react-native`, `phosphor-react-native`, `moti`) stay in `package.json` for future cautious re-enable once verified compatible.
...
- `package.json`: name `forshe`, version `1.2.7`.
- Latest APK build: `519dc1ff-4a36-4756-a78c-8194f49fb942` (v1.2.7 hotfix, queued 2026-04-20, https://expo.dev/accounts/smartbzss/projects/forshe/builds/519dc1ff-4a36-4756-a78c-8194f49fb942).
- Previous APKs `d718bd8a` (v1.2.4), `ed6cce5f` (v1.2.5), `36423651` (v1.2.6) all **crash on launch** — do not distribute.
- Last confirmed-working APK before v1.2.7: `2218683b` (v1.2.3, https://expo.dev/artifacts/eas/5dceXqTVYSAxAvVjmx5Eo5.apk).
